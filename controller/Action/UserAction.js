...
const UserAction = {
    ...
    formatBatchRequest : async function(user){
        console.log(user)
        var result = await Promise.all([CourseAction.getCourseByArrId(user.taken_courses),
                                        CourseAction.getCourseByArrId(user.needed_courses),
                                        CourseAction.getCourseByArrId(user.ip_courses),
                                        CourseAction.getAllCourses(),
                                        ScheduleAction.getScheduleByArrId(user.schedules)]);

        var takenObjArr = result[0];
        var neededObjArr = result[1];
        var ipObjArr = result[2];
        var courseArr = result[3];

        console.log("*******************************Testing the taken ****************************")
        takenObjArr.forEach(res => console.log(res.id));
        console.log("*******************************Testing the taken ****************************")

        if(takenObjArr === undefined){
            takenObjArr = [];
        }
        if(neededObjArr === undefined){
            neededObjArr = [];
        }
        if(ipObjArr === undefined){
            ipObjArr = [];
        }
        ...
        //Check for null
        Object.keys(newUser).forEach(keys => {
            let value = newUser[keys];
            if(value === undefined){
                newUser[keys] = [];
            }
        })
    ...
    saveAcademicProgress : async function(academicProgress){
        var googleId = academicProgress.googleId;
        var taken = academicProgress.taken_courses;
        var ip = academicProgress.ip_courses;
        var needed = academicProgress.needed_courses;


        // await UserDao.updateTakenCourses(googleId, taken);
        // await UserDao.updateIpCourses(googleId, ip);
        // await UserDao.updateNeededCourses(googleId, needed);
        return await UserDao.updateAcademicHistory(googleId, taken, ip, needed)
    },
    ...
    createDemoAccount : async function(user){
        ...
        "IGET 0"
           ...
           //Quarter objects
           var firstQuarter = await QuarterAction.createQuarter({courses : ["AIP 197P", "CSE 112", "CSE 132A"], season : "Fall", year : "2018", schedule : schedule._id});
           var secondQuarter = await QuarterAction.createQuarter({courses : ["CSE 101","MATH 154", "LTLA 105"], season : "Winter", year : "2019", schedule : schedule._id});
           var thirdQuarter = await QuarterAction.createQuarter({courses : [], season : "Spring", year : "2019", schedule : schedule._id});
           var fourthQuarter = await QuarterAction.createQuarter({courses : ["CSE 101","AAS 198", "AAS 87"], season : "Fall", year : "2018", schedule : scheduleTwo._id});
           var fifthQuarter = await QuarterAction.createQuarter({courses : ["AIP 197P", "CSE 112", "CSE 132A"], season : "Spring", year : "2019", schedule : scheduleTwo._id});

   
           var added = await QuarterAction.addQuarterToSchedule(schedule._id, firstQuarter._id);
           var second = await QuarterAction.addQuarterToSchedule(schedule._id, secondQuarter._id);
           var third =  await QuarterAction.addQuarterToSchedule(schedule._id, thirdQuarter._id);
           var fourth = await QuarterAction.addQuarterToSchedule(scheduleTwo._id, fourthQuarter._id);
           var fiftrh = await QuarterAction.addQuarterToSchedule(scheduleTwo._id, fifthQuarter._id);
    ...
    deleteAccount : async function(googleId){
        return await UserDao.removeAccount(googleId);
    }
